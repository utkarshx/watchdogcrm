	...
	switch(type) {
		case 'left':
			formItems.each(function(i,item) {
				$(item).removeClass('noLabel topLabel').addClass('leftLabel');
		...
		case 'none':
			formItems.each(function(i,item) {
				$(item).removeClass('topLabel leftLabel').addClass('noLabel');
				$(item).find('input,textarea').attr('value','').val('').css('color','#000');	// reset default value and clear field
			});
			break;
		case 'top':
		default:
			formItems.each(function(i,item) {
				$(item).removeClass('noLabel leftLabel').addClass('topLabel');
				$(item).find('input,textarea').attr('value','').val('').css('color','#000');	// reset default value and clear field
			});
	}
...
function generateFormJson() {
	...
	return '{"sections":[' + formSections.join(',') + ']}';
}
